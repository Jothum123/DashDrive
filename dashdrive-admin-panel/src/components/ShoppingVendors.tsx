    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-white p-4 rounded-[32px] shadow-soft border border-slate-100/50">
                <div className="flex bg-slate-100 p-1.5 rounded-2xl w-full md:w-fit overflow-x-auto scrollbar-hide border border-slate-200/50">
                    {tabs.map((tab) => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={cn(
                                "px-6 py-2 rounded-xl text-[9px] font-bold font-small-caps tracking-widest transition-all whitespace-nowrap",
                                activeTab === tab
                                ...
                                <tr key={vendor.id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-[10px] font-bold text-slate-400 font-small-caps uppercase">Owner: {vendor.name}</span>
                                                    <span className="w-1 h-1 rounded-full bg-slate-200" />
                                                    <span className="text-[10px] font-bold text-primary font-small-caps uppercase whitespace-nowrap">{vendor.id}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="space-y-1">
                                            <div className="flex items-baseline gap-2">
                                                ...
                                                <span className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest">+12%</span>
                                            </div>
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex items-center justify-between">
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-bold text-slate-400 font-small-caps uppercase tracking-widest leading-none">Global Fee</p>
            ...
            <div className="bg-slate-900 rounded-[32px] p-8 flex flex-col md:flex-row items-center justify-between relative overflow-hidden group">
                ...
                <button className="mt-6 md:mt-0 px-8 py-3 bg-white text-slate-950 rounded-xl text-[10px] font-bold font-small-caps tracking-widest hover:bg-primary hover:text-white transition-all shadow-2xl relative z-10">
                    Global Fee Engine
                </button>
            </div>
        </div>
...
const StatBox: React.FC<{ title: string, value: string, subValue: string, color: string }> = ({ title, value, subValue, color }) => (
    <div className="bg-white p-7 rounded-[40px] shadow-soft border border-slate-100/50 group hover:shadow-xl transition-all">
        <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold text-slate-400 font-small-caps tracking-widest uppercase mb-2">{title}</span>
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-slate-400 font-small-caps uppercase leading-none">{subValue}</span>
            </div>
        </div>
    </div>
);
