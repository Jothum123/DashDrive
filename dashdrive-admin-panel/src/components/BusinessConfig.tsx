            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    ...
                    <div key={i} className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm space-y-8 group hover:border-[#0089D1]/30 transition-all duration-500">
                        ...
                        <button className="w-full py-4 text-[#0089D1] text-[10px] font-black uppercase tracking-widest hover:bg-[#0089D1]/5 rounded-2xl transition-all border border-[#0089D1]/10">Manage {box.title.split(' ')[0]}</button>
                    </div>
                ))}
            </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-50">
                                <th className="pb-6 text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] px-4">Role Designation</th>
                                <th className="pb-6 text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] px-4">Permissions Scope</th>
                                <th className="pb-6 text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] px-4 text-center">Active Users</th>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 text-black">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <span className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em]">Administration</span>
                        <ChevronRight className="w-3 h-3 text-slate-300" />
                        <span className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em]">Config</span>
                    </div>
                ...
                <div className="flex items-center gap-3 p-2 bg-white rounded-[32px] border border-slate-100 shadow-sm">
                    {['Business Setup', 'Configuration', 'Roles & Permissions', 'Settings'].map((tab) => (
                        <button
                            key={tab}
