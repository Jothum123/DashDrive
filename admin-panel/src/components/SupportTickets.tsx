...
export const SupportTickets: React.FC<SupportTicketsProps> = ({ initialTab = 'Active' }) => {
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black text-[#0089D1] uppercase tracking-[0.2em]">Support Triage</span>
                        </div>
    ...
    const renderTabs = () => (
        <div className="flex items-center gap-2 p-1.5 bg-slate-100/80 rounded-[28px] w-fit border border-slate-200/50 backdrop-blur-sm">
            {(['Active', 'Agents', 'Performance', 'Settings', 'Console'] as const).map((tab) => (
                <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={cn(
                        "px-8 py-3.5 rounded-[22px] text-[10px] font-black uppercase tracking-[0.15em] transition-all duration-500 flex items-center gap-3",
                        activeTab === tab
                            ? "bg-white text-slate-900 shadow-sm border border-slate-100"
                            : "text-slate-400 hover:text-slate-600"
                    )}
                >
                    {tab === 'Active' && <MessageSquare className="w-4 h-4" />}
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="flex items-center gap-3 px-8 py-4 bg-slate-900 text-white rounded-[24px] text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/10">
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-100">
                            <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Ticket Information</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {TICKETS_DATA.map((ticket) => (
                            <tr key={ticket.id} className="group hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{ticket.customer}</span>
                                                <div className="w-1 h-1 rounded-full bg-slate-200" />
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{ticket.date}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-10 py-8">
                                    <div className={cn(
                                        "px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest w-fit flex items-center gap-2",
                                        ticket.status === 'Open' ? "bg-blue-50 text-blue-600" :
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="flex items-center justify-between px-4">
                <div className="space-y-1">
                    <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase italic">Support Staff</h3>
                    <p className="text-xs font-medium text-slate-500">Manage support agents and monitor their active workload.</p>
                </div>
                <button className="flex items-center gap-3 px-8 py-4 bg-[#0089D1] text-white rounded-[24px] text-xs font-black uppercase tracking-widest hover:bg-[#007AB8] transition-all shadow-lg shadow-[#0089D1]/20">
                ...
                {AGENTS_DATA.map((agent) => (
                    <div key={agent.id} className="bg-white rounded-[40px] border border-slate-100 p-8 space-y-6 hover:shadow-2xl hover:border-[#0089D1]/20 transition-all duration-500 group">
                        ...
                        <div className="p-4 bg-slate-50 rounded-2xl flex items-center justify-between">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Tickets</span>
                            <span className="text-sm font-black text-slate-900">{agent.ticketsCount}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <button className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">Edit</button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 px-4">
                <div className="bg-slate-900 rounded-[64px] p-12 text-white space-y-10 relative overflow-hidden shadow-2xl">
                    ...
                    <div className="space-y-2 relative z-10">
                        <h3 className="text-2xl font-black tracking-tight uppercase italic">Resolution Trends</h3>
                    ...
                    <div className="h-64 flex items-end gap-3 relative z-10">
                        {Array.from({ length: 12 }).map((_, i) => (
                            <div key={i} className="flex-1 group relative">
                                ...
                                />
                                <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-[8px] font-black text-white/20 uppercase tracking-widest">W{i + 1}</div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[64px] border border-slate-100 p-12 space-y-10 shadow-sm">
                    <div className="space-y-2">
                        <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase italic text-center">Top Performing Agents</h3>
        ...
        <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700 px-4">
                ...
                <div className="bg-white rounded-[48px] border border-slate-100 p-12 space-y-10 shadow-sm">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase italic">Assignment Rules</h3>
                        </div>
                    ...
                    <div className="space-y-6">
                        {[
                            ...
                            <div key={i} className="flex items-center justify-between p-6 bg-slate-50 rounded-[32px] group hover:bg-slate-100 transition-all border border-transparent hover:border-slate-200">
                                <div className="space-y-1">
                                    <h5 className="text-[11px] font-black text-slate-900 uppercase tracking-tight">{rule.label}</h5>
                ...
                <div className="bg-white rounded-[48px] border border-slate-100 p-12 space-y-10 shadow-sm">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase italic">Support Categories</h3>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <button className="flex items-center justify-center gap-2 p-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all">
            ...
            <div className="bg-slate-900 rounded-[64px] p-12 text-white space-y-10 relative overflow-hidden shadow-2xl mt-12">
                ...
                <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
                    ...
                    <div className="flex-1">
                        <h4 className="text-2xl font-black tracking-tight uppercase italic">Automated Notifications</h4>
                    ...
                    <div className="flex flex-col gap-3">
                        <button className="px-8 py-3 bg-[#0089D1] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#007AB8] transition-all">Enable All Alerts</button>
                        <button className="px-8 py-3 bg-white/10 text-white/60 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/20 transition-all">Custom Rules</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    const renderLiveConsole = () => (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="flex items-center justify-between px-4">
                <div className="space-y-1">
                    <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase italic text-primary">Live Operations Console</h3>
            ...
            <div className="bg-slate-950 rounded-[48px] border border-slate-800 shadow-2xl overflow-hidden relative group">
                ...
                <div className="px-10 py-6 bg-slate-900 border-t border-slate-800 flex items-center gap-6">
                    ...
                    <button className="flex items-center gap-3 px-8 py-3 bg-white/5 hover:bg-white/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border border-white/10 group">
            ...
            <div className="max-w-4xl mx-auto p-12 bg-slate-900 rounded-[64px] text-white space-y-10 relative overflow-hidden shadow-2xl">
                ...
                <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
                    ...
                    <div>
                        <h4 className="text-2xl font-black tracking-tight uppercase italic underline decoration-[#0089D1] decoration-4 underline-offset-8">Real-time Triage Logic</h4>
        ...
        <div className="max-w-[1700px] mx-auto space-y-12 pb-20 px-4">
            ...
            <div className="max-w-4xl mx-auto p-12 bg-white rounded-[64px] border border-slate-100 shadow-xl space-y-10 relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row items-center gap-10">
                    ...
                    <div>
                        <h4 className="text-2xl font-black text-slate-900 tracking-tight uppercase italic">Operational Health</h4>
                        ...
                        <div className="flex items-center gap-6 mt-6">
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">SLA Compliant</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-blue-500" />
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Agents Online: 14/15</span>
                            </div>
                        </div>
                    </div>
